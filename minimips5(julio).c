...
int main(){
	...
	struct memoria_inst memInstrucao;
	
	Instrucao instrucoes[256];
	
	memInstrucao->mem_inst=&instrucoes;
	FILE *file_mem;
	int linhas_mem=0;
	char barraN;
	...
	for(int i=0; i<linhas_mem; i++)
	{
		...
		switch (memoria_inst[i].tipo_inst)
		{
			case tipo_R:
				...
				int decimal=0;
				for(int j=3;j>0;j--){
					if (memoria_inst[i].inst_char[j] == '0'){
						decimal +=0;
					}
					else{
							decimal = decimal + 1 * pow(2,j);
					}
				}
				memoria_inst[i].opcode = decimal;
				decimal=0;
				for(int j=6;j>4;j--){
					if (memoria_inst[i].inst_char[j] == '0'){
						decimal +=0;
				...
				memoria_inst[i].rs = decimal;
								
				int bin = atoi(&memoria_inst[i].inst_char[4]);
