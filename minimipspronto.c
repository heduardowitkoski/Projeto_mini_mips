...
void executaUm(Instrucao *instrucoes, int linhas_mem, int *registradores, int *dados, int *pc);
...
int main() {
    ...
    do {
        ...
        switch (menu) {
            case 1:
                if ((file_mem = fopen("programaTestaInstrucoes5.mem", "r")) == NULL) {
            ...
            case 3:
				...
				executaUm(instrucoes, linhas_mem, registradores, dados, &pc);
                break;
...
void voltainstrucao(int *registradores, int reg_backup[][8], int *dados, int dados_backup[][256], int *pc){
	...
	for(int i=0; i<8; i++)
	{
		registradores[i] = reg_backup[*pc][i]; 
	}
	for(int j=0; j<256; j++)
	{
		dados[j] = dados_backup[*pc][j];
	}
